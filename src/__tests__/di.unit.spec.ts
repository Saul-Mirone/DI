...
describe("di", () => {
  ...
  describe("dispose", () => {
    test("dispose basic functionality", () => {
      const container = new Container();
      ...
      const provider = container.provider();
      const service = provider.get(TestService);
    ...
    test("dispose with Symbol.dispose pattern", () => {
      const container = new Container();
    ...
    test("dispose handles services without dispose method", () => {
      const container = new Container();
    ...
    test("dispose handles multiple services", () => {
      const container = new Container();
      ...
      container.add(ServiceA).add(ServiceB);

      const provider = container.provider();
      const serviceA = provider.get(ServiceA);
      const serviceB = provider.get(ServiceB);

      provider.dispose();

      expect(serviceA.disposed).toBe(true);
      expect(serviceB.disposed).toBe(true);
    });

    test("dispose handles services with dependencies", () => {
      const container = new Container();
      ...
      container.add(Dependency).add(Service, [Dependency]);

      const provider = container.provider();
      const service = provider.get(Service);
      const dependency = service.dep;

      provider.dispose();

      expect(service.disposed).toBe(true);
      expect(dependency.disposed).toBe(true);
    });

    test("dispose ignores non-object services", () => {
      const container = new Container();

      const StringService = createIdentifier<string>("string");
      const NumberService = createIdentifier<number>("number");

      container.addValue(StringService, "test");
      container.addValue(NumberService, 42);

      const provider = container.provider();
      provider.get(StringService);
      provider.get(NumberService);
